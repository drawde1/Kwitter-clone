...
export const addMessage = (text, msgParams) => async (dispatch, getState) => {
  return dispatch(_addMessage(text)).then(() => {
    return dispatch(getMessageList(msgParams));
  });
};
...
export const getMessageListByUser = (msgParams, username) => async (
  dispatch,
  getState
) => {
  try {
...
export const likes = (messageId, msgParams) => async (dispatch, getState) => {
  return dispatch(_likes(messageId)).then(() => {
    return dispatch(getMessageList(msgParams));
  });
};

export const profileLikes = (messageId, msgParams, username) => async (
  dispatch,
  getState
) => {
  return dispatch(_likes(messageId)).then(() => {
    return dispatch(getMessageListByUser(msgParams, username));
  });
};
...
export const deleteMessage = (messageId, msgParams) => async (
  dispatch,
  getState
) => {
...
export const deleteLikes = (id, msgParams) => async (dispatch, getState) => {
  return dispatch(_deleteLikes(id)).then(() => {
    return dispatch(getMessageList(msgParams));
  });
};
